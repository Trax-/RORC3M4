class Race
  ...
  def create_entrant(racer)
    race_attributes = self.attributes.symbolize_keys.slice(:_id, :n, :date)
    racer_attributes = racer.info.attributes
    group = get_group(racer)

    entrant = Entrant.new(group: group)
    entrant.race = race_attributes
    entrant.racer = racer_attributes
